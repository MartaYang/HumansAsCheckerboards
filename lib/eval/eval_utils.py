...
def compute_accel_vel(joints):
    ...
    velocities_normed = torch.norm(velocities, dim=2)
    # velocities_normed = torch.concat((velocities_normed, velocities_normed[-1][None]), dim=0)
    acceleration = velocities[1:] - velocities[:-1]
    acceleration_normed = torch.norm(acceleration, dim=2)
    # acceleration_normed = torch.concat((acceleration_normed, acceleration_normed[-1][None].repeat(2,1)), dim=0)
    return acceleration_normed * (30 ** 2) , velocities_normed * (30 ** 2) 
...
def compute_angular_velocity(rotmat):
    """ Computes angular velocity based on successive rotation matrices for batched inputs. """
    N, batch_size, _, _ = rotmat.shape
    angular_velocities = []

    for i in range(1, N):
        # Rotation difference for the entire batch
        R_diff = np.matmul(rotmat[i], np.transpose(rotmat[i - 1], (0, 2, 1)))
        # Axis-angle representation
        angles = np.arccos(np.clip((np.einsum('bii->b', R_diff) - 1) / 2, -1, 1))
        # Avoid division by 0 when angle is close to 0
        small_angle = angles < 1e-3

        # Axis calculation
        axis = np.empty((batch_size, 3))
        axis[:, 0] = R_diff[:, 2, 1] - R_diff[:, 1, 2]
        axis[:, 1] = R_diff[:, 0, 2] - R_diff[:, 2, 0]
        axis[:, 2] = R_diff[:, 1, 0] - R_diff[:, 0, 1]

        # For small angles, use first-order approximation
        omega = 0.5 * axis
        # For larger angles, use the axis-angle formula
        omega[~small_angle] = axis[~small_angle] / (2 * np.sin(angles[~small_angle]).reshape(-1, 1))

        angular_velocities.append(omega * angles.reshape(-1, 1))

    return np.array(angular_velocities)

def compute_angular_accel(rotmat):
    """
    Computes angular acceleration of root orientation.
    Args:
        rotmat (Nx24x3x3).
    Returns:
        Accelerations (N-2,24).
    """
    angular_velocities = compute_angular_velocity(rotmat)
    angular_accelerations = angular_velocities[1:] - angular_velocities[:-1]
    angular_accelerations_normed = np.linalg.norm(angular_accelerations, axis=2)
    return angular_accelerations_normed
...
def compute_rte_absolute(target_trans, pred_trans):
    # Compute the global alignment
    _, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :], fixed_scale=True)
    pred_trans_hat = (
        torch.einsum("tij,tnj->tni", rot, pred_trans[None, :]) + trans[None, :]
...
def compute_roe(target_orientation, pred_orientation):
    # Assuming target_orientation and pred_orientation are both of shape (N, 3, 3),
    # where N is the number of frames.

    # Align the initial orientation 
    initial_alignment = target_orientation[0].transpose(0, 1) @ pred_orientation[0]
    pred_orientation_aligned = torch.einsum('ij,njk->nik', initial_alignment, pred_orientation)

    # Compute the relative rotations between the target and predicted, aligned orientations
    relative_rotations = torch.einsum('nij,njk->nik', target_orientation, pred_orientation_aligned.transpose(1, 2))

    # Convert the rotation matrices to Rodrigues vectors (axis-angle representation)
    rodrigues_vectors = transforms.matrix_to_axis_angle(relative_rotations)

    # Compute the angle of rotation in radians for each Rodrigues vector
    angles_rad = torch.norm(rodrigues_vectors, dim=-1)

    # Convert angles to degrees
    angles_deg = torch.rad2deg(angles_rad)

    # # Compute the Root Orientation Error (ROE)
    # roe = torch.sqrt(torch.mean(torch.pow(angles_deg, 2)))

    return angles_deg.numpy()
    # return roe.item()  # Return a Python float


# for camera
def compute_ate(target_trans, pred_trans):
    # Compute the global alignment
    s, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :])

    pred_trans_hat = (
        s[:, None] * torch.einsum("tij,tnj->tni", rot, pred_trans[None, :]) + trans[None, :]
    )[0]
    ...
    # Normalize it to the displacement
    return ate.numpy(), pred_trans_hat

def compute_ate_s(target_trans, pred_trans):
    # Compute the global alignment
    _, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :], fixed_scale=True)
    ...
    # Compute absolute root-translation-error (RTE)
    ate_s = torch.norm(target_trans - pred_trans_hat, 2, dim=-1)
    
    # Normalize it to the displacement
    return ate_s.numpy(), pred_trans_hat